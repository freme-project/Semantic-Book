...
;(function () {
    ...
    function filter() {
        ...
        settings.currentResults = _.select(settings.items, function (item) {
            ...
            _.each(settings.state.filters, function (facetFilters, facet) {
                if ($.isArray(item[facet])) {
                    var inters = _.intersect(item[facet], facetFilters);
                    if (inters.length === 0) {
                        filtersApply = false;
                    }
                }
                else if (facetFilters.length && _.indexOf(facetFilters, item[facet]) === -1) {
                    filtersApply = false;
                }
            });
            return filtersApply;
    ...
    function order() {
        if (settings.state.orderBy) {
            var $orderEl = $('#orderby_' + settings.state.orderBy);
            var reverse = false;
            if ($orderEl.hasClass('activeorderby')) {
                if ($orderEl.hasClass('activereverseorderby')) {
                    $orderEl.removeClass('activereverseorderby');
                }
                else {
                    $orderEl.addClass('activereverseorderby');
                    reverse = true;
                }
            }
            else {
                ...
                $orderEl.addClass("activeorderby");
            }
    ...
    function createFacetUI() {
        ...
        _.each(settings.facets, function (facettitle, facet) {
            ...
            _.each(settings.facetStore[facet], function (facetFilter, filtername) {
                var item = {id: facetFilter.id, name: filtername, count: facetFilter.count};
        ...
        $('.facetitem').click(function (event) {
            var thisFilter = getFilterById(this.id);
            toggleFilter(thisFilter.facetname, thisFilter.filtername);
            $(settings.facetSelector).trigger("facetedsearchfacetclick", thisFilter);
            order();
            updateFacetUI();
            updateResults();
        });
        ...
        var countHtml = _.template(settings.countTemplate);
    ...
    function getFilterById(id) {
        ...
        _.each(settings.facetStore, function (facet, facetname) {
            _.each(facet, function (facetFilter, filtername) {
                if (facetFilter.id === id) {
    ...
    function updateFacetUI() {
        ...
        _.each(settings.facetStore, function (facet, facetname) {
            _.each(facet, function (facetFilter, filtername) {
                ...
                $("#" + facetFilter.id).html(filteritem);
                if (settings.state.filters[facetname] && _.indexOf(settings.state.filters[facetname], filtername) >= 0) {
                    $("#" + facetFilter.id).addClass("activefacet");
                }
                else {
                    $("#" + facetFilter.id).removeClass("activefacet");
                }
            });
        });
        var countHtml = _.template(settings.countTemplate);
